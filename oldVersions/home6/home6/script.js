...
function showErrorPopup(message) {
    ...
    // Avoid terminal area (left, right, top, bottom with padding)
    let x, y;
    const maxAttempts = 10;
    let attempts = 0;

    do {
        // Randomly choose side (top-left, top-right, bottom-left, bottom-right)
        const side = Math.floor(Math.random() * 4);
        if (side === 0) { // Top-left
            x = padding;
            y = padding;
        } else if (side === 1) { // Top-right
            x = screenWidth - 300 - padding; // 300 is approximate popup width
            y = padding;
        } else if (side === 2) { // Bottom-left
            x = padding;
            y = screenHeight - 100 - padding; // 100 is approximate popup height
        } else { // Bottom-right
            x = screenWidth - 300 - padding;
            y = screenHeight - 100 - padding;
        }
    ...
    if (attempts >= maxAttempts) {
        x = screenWidth / 2 - 150; // Center but adjusted away from terminal
        y = padding;
    }
    ...
    function drag(e) {
        if (isDragging) {
            ...
            currentX = Math.max(padding, Math.min(screenWidth - 300 - padding, currentX));
            currentY = Math.max(padding, Math.min(screenHeight - 100 - padding, currentY));
            ...
            if (currentX >= terminalRect.left - padding && currentX <= terminalRect.right + padding &&
                ...
                currentY = terminalRect.top - 120; // Move above terminal if overlapping
            }
